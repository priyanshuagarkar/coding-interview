...
export async function createSession(req, res) {
    ...
    try {
        ...
        const callId = `session_${Date.now()}_{${Math.randon().toString(36).subString(7)}`;
...
export async function getActiveSessions(_, res) {
    try {
        ...
        console.log("Error in getActiveSessions controler: ", error.message);
        res.status(500).json({ message: "Internal Server Error " });
    }
}
...
export async function getMyRecentSessions(req, res) {
    try {
        ...
        console.log("Error in getMyRecentSessions controler: ", error.message);
        res.status(500).json({ message: "Internal Server Error " });
    }
}
